        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <button type="button" class="btn btn-primary">
                    <a href="{{route('addribbon')}}" style="text-decoration: none;color: white;">Add Ribbon</a>
                </button>
            </ol>
        </div>
        </div>
...
<div class="card " style="padding: 10px">
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title text-center p-2">Ribbons Managment</h3>
    </div>
    ...
    <table id="ribbon" class="table table-bordered table-striped">
    <thead>
        <tr>
        <th>ID</th>
        <th>Usine</th>
        ...
        <td>{{$ribbon->id}}</td>
        <td>{{$ribbon->usine}}</td>
        <td>{{$ribbon->references}}</td>
        <td>{{$ribbon->color}}</td>
        <td class="text-center">{{$ribbon->type}}</td>
        <td class="text-center">{{$ribbon->inQty}} KG</td>
        <td class="text-center">{{$ribbon->outQty}} KG</td>
        <td class="text-center">{{$ribbon->total}} KG</td>
        <td style="color: {{ $ribbon->status === 'Epuisé' ? 'red' : 'green' }};" class="font-weight-bold ">{{ $ribbon->status }}</td>
        <td class="text-center">{{{$ribbon->date}}}</td>
        <td class="text-center">
        <span class="dtr-data">
            <a class="mr-1" href="{{route('showribbon',$ribbon->id)}}"> <i class="nav icon far fa-eye fa-lg text-black"></i></a>   
            <a class="mr-1" href="{{route('editribbon',$ribbon->id)}}"> <i class="nav-icon fas fa-edit fa-lg text-green"></i></a>
            <a href="{{route('deleteribbon',$ribbon->id)}}"
...
$(document).ready(function () {
    ...
    table.buttons().container().appendTo($('#ribbon_wrapper .col-md-6:eq(0)'));
    ...
    @foreach($ribbons as $ribbon)
        var references = "{{$ribbon->references}}";
        var color = "{{$ribbon->color}}";
